...
# 슬라이드 이미지 로드
img_files = sorted(glob.glob('team2/images/*.jpg'))
if not img_files:
    ...
    if pinch_detected:
        if not last_pinch_time or (current_time - last_pinch_time) > 0.3:
            ...
            if pinch_count == 1 and not cropping:
                ...
                cropping = True
                print(f"{tool_type.capitalize()} 시작 위치: {crop_start_point}")
            elif pinch_count == 2 and cropping:
                crop_end_point = (x, y)
                cropping = False
                pinch_count = 0
                print(f"{tool_type.capitalize()} 종료 위치: {crop_end_point}")
                ...
                if 0 <= y_start < y_end <= canvas_size[1] and 0 <= x_start < x_end <= canvas_size[0]:
                    ...
                    if cropped_area.size == 0:
                        ...
                        print(f"{tool_type.capitalize()} 완료. 회전 모드로 전환.")
                else:
                    print("선택 좌표가 범위를 벗어났습니다.")
...
def save_pdf_files():
    ...
    for i in range(total_slides):
        ...
        if slide_base is None:
            print(f"Error: {img_files[i]}를 읽을 수 없습니다.")
            continue
    ...
    if summary_images:
        ...
        summary_pdf.cell(0, 10, "요약 이미지가 없습니다.", 0, 1, 'C')
        summary_pdf.output("summary_final.pdf")
            ...
            for hand in op.multi_hand_landmarks:
                ...
                if current_time - rotation_start_time > rotation_delay:
                    ...
                    if all(is_finger_up(hand, t, p) for t,p in [(4,2),(8,6),(12,10),(16,14),(20,18)]):
                        ...
                        print(f"회전된 이미지가 요약에 추가되었습니다. 총 요약 이미지 수: {len(summary_images)}")
                        rotation_mode = False
                        should_close_rotating_window = True
                        curr_tool = 'laser pointer'
                        print("회전이 확인되었습니다. 레이저 포인터로 전환되었습니다.")
        ...
        for hand in op.multi_hand_landmarks:
                ...
                if (ptime - ctime) > 0.8:
                    ...
                    if curr_tool == selected_tool:
                        if selected_tool == "erase":
                            ...
                            highlight_layer[:, :] = 0
                            curr_tool = "laser pointer"
                            print("모든 그림을 지웠습니다. 레이저 포인터로 전환되었습니다.")
                            push_state()
                        else:
                            curr_tool = "laser pointer"
                            print("툴이 비활성화되었습니다. 레이저 포인터로 전환되었습니다.")
                    else:
                        curr_tool = selected_tool
                        print("현재 선택된 툴:", curr_tool)
                    time_init = True
                    rad = 40
            else:
                time_init = True
            ...
            if color_margin_left <= x <= color_max_x and y <= color_max_y:
                ...
                if selected_color_name:
                    ...
                    print(f"선택된 색상: {selected_color_name}")
                    ...
                    if time.time() > selection_cooldown:
                        if fingers_up_status:
                            ...
                            if var_inits:
                                ...
                                selection_done = True
                                var_inits = False
                                print("선택이 완료되었습니다. 이동 모드로 전환되었습니다.")
            ...
            if curr_tool == "laser pointer" and check_pinch_gesture(hand):
                ...
                if summary_index != -1 and len(summary_images) > summary_index:
                    if not moving_summary:
                        ...
                        print(f"요약 이미지 {moving_summary_index + 1} 선택됨. 이동 준비 중.")
                    else:
                        ...
                        if target_index != moving_summary_index:
                            ...
                            print(f"요약 이미지 {moving_summary_index + 1}와 {target_index + 1}이(가) 교환되었습니다.")
                            push_state()
                        moving_summary = False
                        moving_summary_index = -1
...
while True:
        ...
        for face_landmarks in face_results.multi_face_landmarks:
            ...
            if is_eye_closed(face_landmarks):
                if eye_closed_time is None:
                    ...
                    print("눈을 3초간 감았습니다. PDF가 저장되고 카메라가 종료됩니다.")
                    cap.release()
                    ...
                    if thumb_direction > 0.05:
                        if current_slide < len(img_files) - 1:
                            ...
                            if slide_image is None:
                                print(f"Error: {img_files[current_slide]}를 읽을 수 없습니다.")
                                continue
                            slide_image = cv.resize(slide_image, canvas_size)
                            pen_layer = slide_pen_layers[current_slide].copy()
                            highlight_layer = slide_highlight_layers[current_slide].copy()
    ...
    if mode == 'lecture':
        ...
        if slide_image_current is None:
            print("Error: breaktime.png을(를) 찾을 수 없습니다.")
    ...
    if mode == 'lecture':
        if timer_running:
            ...
            if elapsed_time >= lecture_duration:
                ...
                timer_running = True
                start_time = time.time()
                print("강의 시간이 종료되었습니다. 휴식 모드로 전환됩니다.")
        else:
            display_time = lecture_duration
    else:
        elapsed_time = time.time() - start_time
        ...
        if elapsed_time >= break_duration:
            ...
            start_time = None
            display_time = lecture_duration
            print("휴식 시간이 종료되었습니다. 강의 모드로 돌아갑니다.")
    ...
    if selection_done and curr_tool == "select_move":
        if selected_area_pen is not None:
            ...
            overlay_color_pen = cv.merge((b_pen, g_pen, r_pen))
            mask_pen = a_pen
            cv.copyTo(overlay_color_pen, mask_pen, pen_layer[y1_dst_pen:y2_dst_pen, x1_dst_pen:x2_dst_pen])
        ...
        if selected_area_highlight is not None:
            ...
            mask_highlight = a_highlight
            cv.copyTo(overlay_color_highlight, mask_highlight, highlight_layer[y1_dst_highlight:y2_dst_highlight, x1_dst_highlight:x2_dst_highlight])
    ...
    if mode == 'lecture':
        ...
        if pointer_layer is not None and pointer_layer.size > 0:
            ...
            print("Error: pointer_layer이 비어있거나 None입니다.")
