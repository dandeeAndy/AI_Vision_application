...
def process_frame(frame):
    ...
    # rotation_mode 상태일 때는 landmarks를 여기서 그리지 않음.
    # 회전 모드에서는 main 루프에서 손을 그리지 않고 여기서도 손에 관한 처리만.
    # 즉, 손 랜드마크는 rotation_mode 아닐 때 main loop에서만 그릴 것.
    if rotation_mode:
        # rotation_mode일 때는 draw_landmarks 호출 없음.
    ...
    # rotation_mode가 아닐 때의 처리 로직
    if op.multi_hand_landmarks:
        for hand in op.multi_hand_landmarks:
            # 이 위치에서만 한 번 손 랜드마크를 그림
            ...
            if curr_tool == "draw":
                ...
                if not selection_done:
                    ...
                    # selection_done == True, 영역이 선택된 상태
...
while True:
    ...
    # rotation_mode 아닐 때만 여기서 랜드마크 그리기를 수행.
    # rotation_mode 일 때는 process_frame 내에서 draw_landmarks 호출 안함.
    # 따라서 이중 표기 방지.
    # 이미 process_frame 내에서 multi_hand_landmarks를 기반으로 draw하고 있으므로
    # 여기서는 별도 draw하지 않음.
    ...
    if op.multi_hand_landmarks and op.multi_handedness and not rotation_mode:
        for i, hand_landmarks in enumerate(op.multi_hand_landmarks):
            ...
            if handedness_label == "Left":
                # 여기서는 draw_landmarks 호출 없음 (이미 process_frame에서 그려졌기 때문)
