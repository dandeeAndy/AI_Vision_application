...
image_sizes = []  # (height, width) 튜플을 저장하는 리스트
...
def adjust_summary_images(summary_images, new_image, tool_type):
    if not summary_images:  # 첫 번째 이미지인 경우
        return [new_image.copy()]
    
    # 기준이 될 base_size 계산 (가장 큰 이미지의 크기)
    max_h = max(img.shape[0] for img in summary_images)
    max_w = max(img.shape[1] for img in summary_images)
    
    # tool_type에 따른 scale factor 결정
    scale_factors = {
        "enlarge": 2.0,
        "crop": 1.0,
        "shrink": 0.5
    ...
    # 새 이미지 스케일링
    current_scale = scale_factors.get(tool_type, 1.0)
    if tool_type == "enlarge":
        # enlarge는 원본 크기 유지 (이미 2배로 확대된 상태)
        scaled_new_image = new_image.copy()
        base_size = (new_image.shape[1], new_image.shape[0])
    else:
        # 다른 타입들은 가장 큰 이미지 기준으로 스케일링
        scaled_new_image = cv.resize(new_image, (int(max_w * current_scale), int(max_h * current_scale)))
        base_size = (max_w, max_h)
    
    # 기존 이미지들 스케일링
    scaled_images = []
    for img in summary_images:
        # 원본 이미지의 tool_type 찾기 (이미지에 메타데이터로 저장되어 있다고 가정)
        img_scale = 1.0  # 기본값
        if hasattr(img, 'tool_type'):
            img_scale = scale_factors.get(img.tool_type, 1.0)
        
        target_w = int(base_size[0] * img_scale)
        target_h = int(base_size[1] * img_scale)
        scaled_images.append(cv.resize(img, (target_w, target_h)))
    
    # 새 이미지 추가
    scaled_images.append(scaled_new_image)
    
    # 최대 4개까지만 유지
    if len(scaled_images) > 4:
        scaled_images.pop(0)
            ...
            for hand in op.multi_hand_landmarks:
                ...
                if current_time - rotation_start_time > rotation_delay:
                    ...
                    if is_all_fingers_up(hand):
                        ...
                        # rotated_image = rotate_image(current_cropped_image, rotation_angle)
                        # # 현재 tool_type 확인
                        # tool_type = curr_tool  # curr_tool은 "crop", "enlarge", "shrink" 중 하나
                        # # summary_images 업데이트
                        # summary_images = adjust_summary_images(summary_images, rotated_image, tool_type)
                        # rotation_mode = False
                        # should_close_rotating_window = True
                        # curr_tool = 'laser pointer'
                        # print("Rotation confirmed. Switched back to laser pointer.")
...
while True:
    ...
    if summary_images:
        ...
        for idx, img in enumerate(summary_images[-4:]):
            ...
            # 기준 크기에 대한 비율 계산
            ratio = min(section_width/(max_W * scale), summary_height/(max_H * scale))
