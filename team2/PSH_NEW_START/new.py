...
while True:
    ...
    if op.multi_hand_landmarks:
        for i in op.multi_hand_landmarks:
            ...
            if curr_tool == "draw":
                if index_raised(yi, y9):  # 검지 손가락이 올라가 있을 때
                    if not drawing:  # 그리기 시작
                        start_x, start_y = x, y  # 시작점 저장
                        drawing = True
                    else:
                        # 임시 레이어에 미리보기 선 표시 (옵션)
                        temp_layer = pen_layer.copy()
                        cv.line(temp_layer, (start_x, start_y), (x, y), (0, 0, 255), thick_pen,8)
                        # temp_layer를 화면에 표시
                else:  # 검지 손가락이 내려갔을 때
                    if drawing:  # 그리기 종료
                        # 실제 선 그리기
                        cv.line(pen_layer, (start_x, start_y), (x, y), (0, 0, 255), thick_pen,8)
                        drawing = False
                ...
                if index_raised(yi, y9):
                    ...
                    if var_inits:
                        ...
                        if selected_area.size != 0:
                            # 'Additional Materials' 창에 선택된 영역 표시
                            cv.imshow("Additional Materials", selected_area)
                            # 크롭된 이미지 저장
                            cropped_images.append(selected_area)
