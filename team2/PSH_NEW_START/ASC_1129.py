...
NOTE_HEIGHT = 200  # 핵심노트 높이
MAIN_HEIGHT = SCREEN_HEIGHT - NOTE_HEIGHT  # 메인 창들의 높이

# 창 위치 설정
cv.namedWindow('Lecture', cv.WINDOW_NORMAL)
cv.namedWindow('Webcam', cv.WINDOW_NORMAL)
cv.namedWindow('Note', cv.WINDOW_NORMAL)

# 창 크기 고정
cv.resizeWindow('Lecture', SCREEN_WIDTH // 2, MAIN_HEIGHT)
cv.resizeWindow('Webcam', SCREEN_WIDTH // 2, MAIN_HEIGHT)
cv.resizeWindow('Note', SCREEN_WIDTH, NOTE_HEIGHT)

# 창 위치 설정
cv.moveWindow('Lecture', 0, 0)
cv.moveWindow('Webcam', SCREEN_WIDTH // 2, 0)
cv.moveWindow('Note', 0, MAIN_HEIGHT)
...
slide_image = cv.resize(slide_image, (SCREEN_WIDTH // 2, MAIN_HEIGHT))
...
note_layer = np.zeros((NOTE_HEIGHT, SCREEN_WIDTH, 3), dtype=np.uint8)  # 전체 화면 너비의 노트 레이어
...
cap.set(cv.CAP_PROP_FRAME_WIDTH, SCREEN_WIDTH // 2)
cap.set(cv.CAP_PROP_FRAME_HEIGHT, MAIN_HEIGHT)
cropped_images = []
...
while True:
    ...
    if not ret:
        break
    frm = cv.flip(frm, 1)
    frm = cv.resize(frm, (SCREEN_WIDTH // 2, MAIN_HEIGHT))   # 웹캠 프레임을 800x800으로 크기 조정
            ...
            if curr_tool == "draw":
                if index_raised(yi, y9):
                    if prevx != 0 and prevy != 0:
                        if is_note_area:
                            # 노트 레이어의 상대 좌표로 변환
                            note_y = y - MAIN_HEIGHT
                            prev_note_y = prevy - MAIN_HEIGHT
                            if 0 <= note_y < NOTE_HEIGHT:  # 범위 체크
                                cv.line(note_layer, (prevx, prev_note_y), (x, note_y), 
                                    (0, 0, 255), thick_pen)
                        else:
                            # 슬라이드 영역 그리기
                            if y < MAIN_HEIGHT:  # 범위 체크
                                cv.line(pen_layer, (prevx, prevy), (x, y), 
                                    (0, 0, 255), thick_pen)
                    prevx, prevy = x, y
                else:
                    prevx, prevy = 0, 0
